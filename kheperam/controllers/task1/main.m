...
while (wb_robot_step(TIME_STEP) ~= -1) & should_run
    ...
    % If we are near a wall:
        ...
        left_motor_dist = 8 * wb_differential_wheels_get_left_encoder()/100;
        right_motor_dist = 8 * wb_differential_wheels_get_right_encoder()/100;
        ...
        theta = theta - 0.5 * (left_motor_dist - right_motor_dist)/(26.5);
